...
def plot_distribution(df,path_fig,name):
    ...
    for error_rate in list(set(df["error_rate"].to_list())):
        ...
        for n, group in df_e.groupby("n"):
            ...
            if error_rate == 0.01:
                ax.scatter(X,Y_norm,color=color,marker='o',s=40,label="$n=%i$"%n)
            elif error_rate == 0.001:
                ax.scatter(X,Y_norm,edgecolors=color,marker='o',facecolors='white',s=40)
            i += 1

    ax.set_xlabel("maximum stack ($m$)",fontsize=8,labelpad=0.5)
    ax.set_ylabel("survival function $(S_M(m))$",fontsize=8,labelpad=0.5)
